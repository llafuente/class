(function (exports) {
    ...
    exports.$interface = __interface = function (name, methods) {
        var $self = function () {
    ...
    exports.method = __method = function ($class, name, method) {
        ...
        if (abs) {
            ...
            } else if (abs.lambda.length < method.length) {
                console.warn("(node-class) [" + name + "] method has: " + method.length + " parameter(s) expected: " + abs.lambda.length);
            }
            //remove
    ...
    module.exports["interface"] = exports.Interface = function (name, definition) {
        ...
        cls = __interface(name, definition);

        if (_extends) {
            for (i = 0, max = _extends.length; i < max; ++i) {
                __extends(cls, _extends[i]);
            }
        }

        if (_implements) {
            for (i = 0, max = _implements.length; i < max; ++i) {
                __extends(cls, _implements[i]);
            }
        }
