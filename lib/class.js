(function (exports, browser) {
    ...
    function set_log_level(name, level, color) {
        log_levels[name] = [level, color];
        ...
        return (exports[name] = function () {
            if (log_level <= level) {
                var arr,
                    ...
                    ms = d.getTime(),
                    date = d.toUTCString(),
                    seconds_from_start = ts_format((ms - ___program_start.getTime()) * 0.001),
                    args;
    ...
    function remove_log_level(name) {
        delete log_levels[name];
        delete exports[name];
    }

    function set_log_function(l) {
        log = l;
    }

    // default config
    set_log_function(console.log);
    set_log_level("error",    5,  "0;31");
    __warning = set_log_level("warning",  4,  "1;33");
    set_log_level("info",     3,  "1;36");
    set_log_level("debug",    2,  "01;35");
    __verbose = set_log_level("verbose",  1,  "01;32");
    ...
    __instanceof = function (cls, cls_name) {
        ...
        $extended = (cls.$extended || (cls.prototype && cls.prototype.$extended ? (cls.prototype.$extended || null) : null));
     ...
     * @params {Boolean} functions
     */
    __remove_key_prefixed = function (obj, prefix) {
        ...
        throw new Error("remove_key_prefixed(object) given[" + __typeof(obj) + "]");
    };
    ...
    */
    ...
    Class = function (name, properties, methods) {
        ...
        if (class_names.indexOf(name) !== -1) {
            throw new Error("Class name already exists: " + name);
        }
        //</security>
        ...
        $self = function (options) {
            ...
            for (i = 0; i < max; ++i) {
                k = $properties[i];
                autoset_prop = $autoset && options && options[k];

                descriptor = __clone($data_properties[k]);
        ...
        $self.implements = function (methods, finale) {
            ...
            for (i in source) {
                ...
                //<debug>
                stype = __typeof(source[i]);
                if (stype !== "function") {
                    __warning(i + " is not a function");
                    continue;
                }
                ...
                if (target[i] !== undefined) {
                    ...
                    target[i] = (function () {
                        ...
                        return function () {
                            ...
                            this.parent = parent;
                            __verbose("set parent, and save the old one", swap_parent, fn, i);
         ...
         * TODO clone the array?
         */
        $self.get_data_properties = function () {
        ...
        $self.extends = function (cls, override_properties, extend_static) {
            ...
            $__extended.push(cls.prototype.$name);

            var methods = cls.get_methods(),
                properties = cls.get_properties(),
                data_properties = cls.get_data_properties(),
            ...
            __verbose("extending methods", methods);
            ...
            for (i = 0; i < max; ++i) {
                ...
                __verbose("new option: ", name, cls.prototype[name]);

                if (override_properties === false) {
                    if ($properties.indexOf(name) === -1) {
                        $properties.push(name);
                        $data_properties[name] = data_properties[name];
                    }
                } else {
                    $properties.push(name);
                    $data_properties[name] = data_properties[name];
        ...
        $self.properties = function (properties) {
            ...
            for (key in properties) {
                ...
                // <debug>
                if ($data_properties[key]) {
                    __warning("property[" + key + "] redefinition");
                }
                // </debug>
                $data_properties[key] = {
        ...
        $self.alias = function (dst_method_name, src_method_name) {
            // <debug>
            __assert_arg(dst_method_name, "string", 1);
            __assert_arg(src_method_name, "string", 2);
            // </debug>
        ...
        $self.hide = function (properties) {
            ...
            for (i = 0; i < max; ++i) {
                if (properties[i] !== "__construct") {
                    k = properties[i];
                    fn = $self.prototype[properties[i]];
                    delete $self.prototype[properties[i]];
                    // todo, hide properties even after equal
                    defineProperty($self.prototype, properties[i], {
                        value: fn,
                        writable : false,
                        enumerable : false,
                        configurable : false
                    ...
                    $data_properties[k].enumerable = false;
                }
            }
    ...
    exports.remove_key_prefixed   = __remove_key_prefixed;
    exports.log_level             = log_level;

    exports.set_log_level         = set_log_level;
    exports.remove_log_level      = remove_log_level;
    exports.set_log_function      = set_log_function;
