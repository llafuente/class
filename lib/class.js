(function ($) {
    ...
    function assert_types(val, type, position) {
        ...
        if(thr) {
            throw "argument[" + (position || "?") + "] must be " + type + "and is [" +  $.typeof(val) +  "] -> " + val;
        }
    }
            ...
            $[levels[i]] = (function () {
                ...
                return function () {
                    var color = "\x1B[" + colors[level] + "m",
                        ...
                        common = String.sprintf("%s[d%'06f][%s][p%s]%24s|", date, ms - ___delta, seconds_from_start, process.pid, where),
                        args = [color, common].concat(slice.call(arguments));
                    ...
                    if (this.log_level > level) {
                        ...
                        if (arr[2] !== undefined) {
                            cut = arr[2].indexOf("(");
                            if (cut !== -1) {
                                where = arr[2].substring(cut + 1, arr[2].lastIndexOf(")"));
                                cut = where.lastIndexOf("/") + 1;
                                where = where.substring(cut);

                            } else {
                                cut2 = arr[2].lastIndexOf("/") + 1;
                                where = arr[2].substring(cut2);
                            }
    ...
    $.clone = function (obj, recursive) {
        ...
        case "function":
            return obj;
        default:
            throw new Error("type cannot be cloned");
        }
    };
    ...
    */
    (function () {
        $.Class = function (name, options, methods) {
            ...
            assert_types(options, "object", 2);
            assert_types(methods, ["null", "object"], 3);

            /*
            // we can remove new right ?
            if (!(this instanceof $.Class)) {
                return new $.Class(name, options, methods);
            }
            */
            ...
            $self = function (options) {
                assert_types(options, ["null", "object"], 1);
                // abstract check
                var i,
                    max = $abstract.length,
                    found,
                ...
                max = $options_list.length;

                $.__log("cloning options");

                $.remove_key_prefixed(options, "__");
                ...
                Object.merge(this, options, false, true, true);
            ...
            $self.prototype.$extended = [ "Class" ];
            $self.prototype.$myself = $self; // this way we can point to the $.Class instead the instanced

            $self.prototype.serialize = function (export_private) {
            ...
            $self.prototype.deserialize = function (options, import_private) {
            ...
            $self.get_methods_list = function () {
                return $methods_list;
            };

            $self.get_abstract_methods_list = function () {
                return $abstract;
            };

            $self.get_final_methods_list = function () {
                return $finale_list;
            };
            /**
            ...
            $self.extends = function (cls, override_properties) {
                ...
                var methods = cls.get_methods_list(),
                    options = cls.get_options_list(),
                    abst = cls.get_abstract_methods_list(),
                    finals = cls.get_final_methods_list(),
                    i,
                    max = methods.length,
                    name = null;
                ...
                $.__log("extending options", options);
                max = options.length;
                for (i = 0; i < max; ++i) {
                    name = options[i];
             ...
             */
            $self.get_options_list = function () {
                return $options_list;
            };
             ...
             *
             * resursive example:
             * options({vector: {x:0}}); // set vector x
             * options({vector: {y:0}}); // set vector with only "y"
             * options({vector: {y:0}}, true); // add y to the vector, so xy :)
             *
             * @param Object options
             * @param Boolean recursive use it when you want to add more options to existing "object properties"
             * @returns this for chaining
             */
            $self.options = function (options, recursive) {
                assert_types(options, ["object"], 1);
                assert_types(recursive, ["null", "boolean"], 2);

                recursive || (recursive = false);
                var key = null;
                for (key in options) {
                    $options_list.push(key);
                    if (!recursive) {
                        this.prototype[key] = options[key];
                    }
                }
                if (recursive) {
                    Object.merge(this.prototype, options, false, true, true);
                }
            ...
            $self.alias = function (fn_name, fn_name2) {
                ...
                if (typeof this.prototype[fn_name2] != "function") {
                    throw "function[" + fn_name2 + "] not found, so cannot be aliased";
                }
        ...
        $.Events.implements({
            ...
            on : function (event, fn, internal, times) {
                ...
                if ($.typeof(event) == "object") {
                    for (ev_name in events) {
                        this.addEvent(ev_name, events[ev_name]);
                    }
                    return this;
                }

                event = removeOn(event);
            ...
            off : function (event, fn) {
                ...
                if (events && !fn.$ev_internal) {
                    ...
                    if (index !== -1) {
                        events[index].__detach && events[index].__detach();
